    ...
    order by us_l3code asc`;
      ...
      getData({ credentials, opts: { abortController }, query })
        .then((res) => {
          ...
          let array_with_city_list = res.map((e) => ({
            ...e,
            city_list_array: e.city_list.split(','),
          }));
      ...
      <Grid item className={classes.sideBar}>
        <div className={classes.lengthCount}>
          <Typography
            ...
            variant='h5'
            component='h2'
            className={classes.listItemsLength}
          >
            {listItems.length} Ecoregions
          </Typography>
        </div>
        ...
        <div>
          {listItems.map((item, index) => (
            <Card
              key={index}
              onMouseOver={(e) => toggleSelectedItem(item, e)}
            ...
            >
              <CardActionArea>
                ...
                <Typography
                  ...
                  gutterBottom
                >
                  <CameraAltIcon className={classes.cameraIcon} /> Flickr User{' '}
                  {item.image_author_4}
                </Typography>
                <CardContent>
                  <Box display='flex' alignItems='center' justifyContent='space-between'>
                    ...
                    <div
                      ...
                      style={{ backgroundColor: getRGBvalue(item.us_l4name) }}
                    />
                  </Box>
                  <ClampLines
                  ...
                  <Box className={classes.citiesAndPop}>
                    <Typography variant='body2'>
                      Largest cities that touch this ecoregion:{' '}
                      {item.city_list_array.slice(0, 5).toString()}{' '}
                    </Typography>
                    <Typography variant='body2'>
                      Total population living in this ecoregion: {item.ecoregion_pop}{' '}
                    </Typography>
                  </Box>
                </CardContent>
                <CardActions>
